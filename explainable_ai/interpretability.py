...
class Interpretation(Understanding):
    ...
    def __init__(self,
        ...
        super().__init__(arguments_used, model_to_understand, data_source, feature_names, target_name, task_id)
    ...
    def _uai_feature_importance(self, *args, **kwargs):
        n_feature = kwargs.get('n_feature') if kwargs.get('n_feature') else 100000
        target = self.feature_names[-1]
        classes = ['regression']
        if self.data_source.dtypes[target] not in [np.float32,np.float64]: # classification problem
            classes = self.data_source[target].unique()
        sorted_idx = np.argsort(self.feature_importance)[::-1]
        
        result = [] # each result is in the format (name_class, n_imp, n_names)
        if len(self.feature_importance.shape) == 1: # problema de regressao
            self.feature_importance = [np.array(self.feature_importance)]
            sorted_idx = [np.array(sorted_idx)]

        for name_class, fi_class,i_sorted_idx in zip(classes, self.feature_importance,sorted_idx):
            n_imp = [fi_class[i] for i in i_sorted_idx[:n_feature]]
            n_names = [self.feature_names[i] for i in i_sorted_idx[:n_feature]]
            result.append((name_class, n_imp,n_names))
